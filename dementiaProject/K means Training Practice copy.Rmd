...
test <-read.csv('./Data/FinalData.csv')
...
training.data
scale(test[-5])
                cdr.testkmeans <-scale(test[-5])
summary(cdr.testkmeans)
cdr.testkmeans
nrow(cdr.testkmeans)
cdr.testkmeans
```
...
cdr.testkmeans <-  kmeans(cdr.testkmeans[,-5], centers = 3, iter.max = 10000)
...
```{r}
          ...
          testCDR$cluster <- as.factor(cdr.testkmeans$cluster)
